...
1.) Calling of TypeScript functions execute as expected. The passing of string, integer, boolean
and class (within our REPL code) instances work as expected.
```
...
* `BuiltInFunction` holds a reference to the actual TypeScript function to be invoked at runtime. This TypeScript function must have the following form:
	* `funcName(... args: number[]) : number { } `
...
```
And so, the heap is simply an array of `Instance`:
`heap: Array<Instance>`
