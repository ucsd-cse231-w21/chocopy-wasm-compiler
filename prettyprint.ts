...
function prettyPrintDictionary(result: any, repl: BasicREPL, currentEle : any){
  ...
  for (i = 0; i < hashtableSize; i++){
    ...
    if (hash_entry != 0){
      ...
      // const key = view[hash_entry/4 + 0];
      // const value = view[hash_entry/4 + 1];
      // const next_ptr = view[hash_entry/4 + 2];
...
function printDictionaryLLHelper(result: any, baseIndex: number, repl: BasicREPL, view: any, currentEle: any){
  ...
  switch(result.value_type.tag){
    ...
    default:
      ...
      ele.innerHTML = "<b class='tag'>" + ele_key.value + ": </b><p class='val'>" + ele_val.value + "</p>";
  }
